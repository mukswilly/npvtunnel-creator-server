...
func TestRedeemMultipleConfigsReturnsBundle(t *testing.T) {
	...
	writeConfigs(t, dir, []ConfigEntry{
		{ConfigID: testCID, Config: json.RawMessage(`{"name":"one","type":"V2RAY"}`)},
		{ConfigID: secondCID, Config: json.RawMessage(`{"name":"two","type":"SSH"}`)},
	})
	state, err := NewStateWithDir(dir)
	if err != nil {
		t.Fatal(err)
	...
	state.PublicIssuerURL = "https://issuer.example/v1/issue"
	const tokenStr = "multi-config-token"
	if err := state.AddRedemptionToken(RedemptionToken{
		Token: tokenStr, ConfigIDs: []string{testCID, secondCID},
		RemainingRedemptions: 2, CreatedAt: time.Now().UTC().Format(time.RFC3339),
	}); err != nil {
		t.Fatal(err)
	}
	...
	ts := newTestServerWithState(t, state)
	defer ts.Close()
	resp := postRedeemRaw(t, ts, RedeemRequest{
		V: 1, Token: tokenStr,
	...
	var bundle redeemBundle
	if err := json.NewDecoder(resp.Body).Decode(&bundle); err != nil {
		t.Fatal(err)
	}
	if bundle.V != 1 || len(bundle.Envelopes) != 2 {
		t.Fatalf("bundle = %#v", bundle)
	}
	wantIDs := []string{testCID, secondCID}
	for i, encoded := range bundle.Envelopes {
		wire, err := base64.RawURLEncoding.DecodeString(encoded)
		if err != nil {
			t.Fatalf("decode envelope %d: %v", i, err)
		}
		decoded, err := decodeEnvelopeWire(wire)
		if err != nil {
			t.Fatalf("parse envelope %d: %v", i, err)
		}
		if decoded.Header.ConfigID != wantIDs[i] {
			t.Errorf("envelope %d configId = %q, want %q", i, decoded.Header.ConfigID, wantIDs[i])
		}
	}
	if got := state.LookupRedemptionToken(tokenStr).RemainingRedemptions; got != 1 {
		t.Errorf("remaining redemptions = %d, want 1", got)
	}
}
...
// invalidCompressedPubkey returns a correctly sized, off-curve P-256 point.
