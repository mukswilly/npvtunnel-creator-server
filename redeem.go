...
func (s *Server) handleRedeem(w http.ResponseWriter, r *http.Request) {
	...
	configIDs := token.configIDs()
	if len(configIDs) == 0 {
		writeRedeemError(w, http.StatusInternalServerError, "server_error",
			"token has no registered configs")
		return
	}
	envelopes := make([][]byte, 0, len(configIDs))
	for _, configIDText := range configIDs {
		cfgEntry := s.state.ConfigByID(configIDText)
		if cfgEntry == nil {
			writeRedeemError(w, http.StatusInternalServerError, "server_error",
				"token references a configId that's no longer registered")
			return
		}
		configID, decodeErr := b64url.DecodeString(configIDText)
		if decodeErr != nil || len(configID) != envelopeConfigIDLen {
			writeRedeemError(w, http.StatusInternalServerError, "server_error",
		...
		envelopes = append(envelopes, mintRes.EnvelopeBytes)
	}
	...
	auditEmit(s.logger, s.state.AuditSalt,
		"redeem.granted",
		req.RecipientPubkey,
		shortBase64(configIDs[0]),
		"", "NONE", false, 0,
		"token", shortBase64(req.Token),
		"label", token.Label,
		"configCount", len(configIDs),
	...
	if len(envelopes) == 1 {
		...
		_, _ = w.Write(envelopes[0])
		return
	}
	encoded := make([]string, len(envelopes))
	for i, envelope := range envelopes {
		encoded[i] = base64.RawURLEncoding.EncodeToString(envelope)
	}
