...
// b64url is base64.URLEncoding with padding stripped — the base64url
// alphabet with no padding, used for every base64url field on the wire.
...
//
// P1363 is the fixed-width signature form the wire format uses.
...
// ──────────────────────────────────────────────────────────────────
// Issuance request/receipt signing — must match the recipient's signing input
// ──────────────────────────────────────────────────────────────────

// issueRequestSigningInput reconstructs the bytes that were signed to
// produce IssueRequest.RequestSignature. Must match the signing input
// the request was produced with, byte-for-byte.
//
// The "configId" position used to be a SHA-256(envelopeBytes) value
// labeled "configFp"; the field name changed when the
// routing key moved to envelope-header configId (see the ConfigEntry
// doc comment in state.go). The signing-string layout — pipe-delimited fields in
...
// are unambiguous in practice. Even a malicious caller stuffing a '|'
...
// caller's OWN devicePk. The string exists only to bind the signature.
