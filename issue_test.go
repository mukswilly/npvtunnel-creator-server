...
func TestIssueRoundTrip(t *testing.T) {
	...
	registerTestConfig(state, configID)
...
func registerTestConfig(state *State, configID string) {
	state.mu.Lock()
	defer state.mu.Unlock()
	if state.configs == nil {
		state.configs = make(map[string]*ConfigEntry)
	}
	state.configs[configID] = &ConfigEntry{
		ConfigID: configID,
		Config:   json.RawMessage(`{"name":"test","address":"vpn.example:443","type":"V2RAY","v2rayProfile":{"server":"vpn.example","serverPort":"443"}}`),
	}
}
