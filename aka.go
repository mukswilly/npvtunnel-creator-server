...
// androidKeyAttestationVerifier parses Android Keystore's hardware
// attestation evidence and extracts a Verdict.
//
// What it verifies:
...
func (v *androidKeyAttestationVerifier) Verify(blob AttestationBlob) (Verdict, error) {
	...
	// Anchor the chain at a Google root via x509.Verify. The chain
	// passed by Android Keystore lists the leaf first and the root
...
// each subsequent cert is its issuer (Android Keystore's standard
