...
// share-link tokens.
//
// Systemd runs this binary as the issuer API. A creator uses the installed
// npv-creator launcher, which always opens the management dashboard. Management
// is intentionally not exposed as a collection of command-line operations.
package main
...
func main() {
	// The installer exposes this friendly command specifically for the
	// interactive operator console. It must keep working with menu flags such as
	// -state-dir; otherwise those flags would be mistaken for server flags.
	if invokedAsCreatorMenu(os.Args[0]) {
		os.Exit(runMenuSubcommand(os.Args[1:]))
	}
	...
	if handled, code := dispatchCommand(os.Args[1:]); handled {
		os.Exit(code)
	}
	...
	if *stateDir == "" {
		...
		if state.KeyWasCreated() {
			logger.Warn("a new creator signing key was generated; back it up from the dashboard",
...
func invokedAsCreatorMenu(program string) bool {
	return filepath.Base(program) == "npv-creator"
}

// dispatchCommand keeps the creator-facing binary surface deliberately small.
// The underscored service entry point exists only for the dashboard's narrow
// sudo boundary and is not presented as an operator command.
func dispatchCommand(args []string) (handled bool, code int) {
	if len(args) == 0 {
		return false, 0
	}
	switch args[0] {
	case "__dashboard":
		return true, runMenuSubcommand(args[1:])
	case "__service":
		return true, runServiceSubcommand(args[1:])
	case "version", "-v", "--version":
		return true, runVersionSubcommand()
	case "-h", "--help":
		return false, 0
	default:
		...
		fmt.Fprintf(os.Stderr,
			"creator-server: %q is not available from the command line\n"+
				"Open the management dashboard with: npv-creator\n",
			args[0])
		return true, 2
	}
}
