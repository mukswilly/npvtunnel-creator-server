...
func TestRepairServiceUnitAdoptsSettingsAndHardensUnit(t *testing.T) {
	unitPath := t.TempDir() + "/creator-server.service"
	legacy := `[Unit]
Description=legacy creator service
[Service]
ExecStart=/usr/local/bin/creator-server -addr :8443 -state-dir /srv/creator -domain issuer.example -acme-email owner@example.com -public-issuer-url https://issuer.example:8443/v1/issue
`
	if err := os.WriteFile(unitPath, []byte(legacy), 0o600); err != nil {
		t.Fatal(err)
	}
	f := &fakeController{}
	var out bytes.Buffer
	if err := repairServiceUnit(f, unitPath, &out); err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
		"User=creator",
		"Group=creator",
		"-state-dir /srv/creator",
		"-domain issuer.example",
		"-acme-email owner@example.com",
		"-addr :8443",
		"-public-issuer-url https://issuer.example:8443/v1/issue",
		"CapabilityBoundingSet=CAP_NET_BIND_SERVICE",
	} {
		if !strings.Contains(f.installedUnit, want) {
			t.Errorf("repaired unit missing %q\n---\n%s", want, f.installedUnit)
		}
	}
