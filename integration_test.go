...
func TestIssueAlwaysOnRateLimit(t *testing.T) {
	...
	registerTestConfig(state, req.ConfigID)
...
func TestDecodeAppRegistrationFormats(t *testing.T) {
	bodyJSON := `{"name":"x","address":"h:443","type":"V2RAY","v2rayProfile":{"server":"h","serverPort":"443","password":"npvs1:AAAA"}}`
	bundle := `{"kind":"npv-config-registration","v":1,"config":` + bodyJSON + `}`
	b64 := b64url.EncodeToString([]byte(bundle))

	for _, in := range []string{b64, bundle, "  " + b64 + "  "} {
		got, _, err := decodeConfigRegistration(in)
		if err != nil {
			t.Fatalf("decodeConfigRegistration(%.16q...): %v", in, err)
		}
	...
	for _, bad := range []string{
		...
		if _, _, err := decodeConfigRegistration(bad); err == nil {
			t.Errorf("decodeConfigRegistration(%q): expected error, got nil", bad)
		}
	}
}
