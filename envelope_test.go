...
func TestCanonicalJSONEscapesControlChars(t *testing.T) {
	...
	// the client emitter (lowercase hex padded to 4 digits).
	want := "{\"k\":\"a\\nb\\tc\\\"d\\\\e\\u0001f\"}"
	if string(out) != want {
		t.Errorf("got %s\nwant %s", out, want)
	}
...
//
// Independent re-implementation of the §6.5 RECEIVER step (mirroring
// what client's the client's unwrap routine does, but in pure
// Go for testing). Confirms the minter's wrap step produces bytes
// that the documented receiver flow can recover the DEK from.
