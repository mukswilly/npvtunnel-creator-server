...
// recipient app parses it and uses it directly through the
// normal connect path.
//
...
type ConfigEntry struct {
	...
	// Config is the full config-body JSON the issuer ships to recipients,
	// returned verbatim. Shape is the config-body JSON the app expects:
	//
...
// defaultIssuanceLimitPerHour is the ALWAYS-ON per-(devicePk, configId)
// issuance cap applied when no attestation policy raises it — i.e. on a
// default deployment with attestation off. It is deliberately more
// generous than defaultMaxIssuancesPerHour: configuring an attestation
// policy signals the creator wants tighter control, whereas this baseline
// must not trip a legitimate user on a flaky network. Because the key is
// the device pubkey (not the source IP), this is safe for users sharing
// one carrier / CGNAT address. Set to 0 only if you truly want no issuance
// limit — but the default deployment should always have one.
...
//	observe  Log what the recipient claimed (platform, token presence), never block.
//	         Useful as a learn-the-audience phase before turning on enforcement.
//	soft     Accept all requests, but shorten TTL for recipients that don't claim
...
//	         attestation. Locks out recipients on devices that can't produce a
//	         Play Integrity / App Attest token. Not appropriate for sideload-
//	         heavy audiences.
//
// What this does NOT yet do:
//
// The issuer currently checks whether the recipient *claimed* attestation
// (Attestation.Platform != "NONE"). It does NOT verify the token by calling
// Google Play Integrity API / Apple App Attest. A malicious recipient can
...
// strongest signal available, and "strict" still keeps out recipients that
...
type AttestationPolicy struct {
	...
	//     (preserves back-compat for deployments that haven't
	...
	// against the recipient's attestation token. Empty (default) skips
	// verification — the policy modes still gate on "claimed" vs
	// "not claimed" exactly as in the claimed-only fallback.
	//
	// Recognized values:
	...
	// verifier knob — those keep behaving as before until the
	...
	// the gate will reject all iOS recipients. Don't pair them.
...
type State struct {
	...
	// CreatorSigningKey signs issuance receipts. Persisted under
	...
	// configsPath is where configs.json lives. Empty in tests / no-dir
	// mode. configsMtime is the file mtime at last successful (or last
	// seen) load; a newer mtime triggers a hot-reload from the handlers
	// (see ReloadConfigsIfChanged) so editing configs.json takes effect
	// without a restart.
	configsPath  string
	configsMtime time.Time

	// keyCreated is true when this run generated a fresh creator-key.pem
	// (first run) rather than loading an existing one. main() uses it to
	// print a prominent "back up your key" banner exactly once.
	keyCreated bool
...
// KeyWasCreated reports whether this run generated a fresh creator signing
// key (first run) rather than loading an existing one.
...
// The bool return is true when a fresh key was generated (first run).
...
func loadConfigsFile(path string) (map[string]*ConfigEntry, error) {
	...
	for i := range list {
		...
		// connect 404'ing later (a class of operator-typo bug).
		...
		// Config must JSON-decode to an object (the config-body shape).
		...
		if entry.AttestationPolicy != nil {
			...
			if entry.AttestationPolicy.Verifier != "" {
				...
				// iOS recipient silently failing attestation.
...
// When false, /v1/issue should fall back to the stub behavior.
...
// ReloadConfigsIfChanged re-reads configs.json from disk when its mtime is
// newer than the last load, validating BEFORE swapping so a bad edit keeps
// the last-good registry live. Called from the issue/redeem handlers so an
// operator editing configs.json takes effect without a restart (mirrors
// ReloadRedemptionTokensIfChanged; cheap one os.Stat on the common path).
//
// A parse/validation error is returned (the caller logs it) and the
// in-memory registry is left UNTOUCHED — an operator typo never takes the
// issuer down. The bad version's mtime is recorded so the warning fires
// once per bad save, not once per request. A removed file is treated as
// "no change" (last-good stays live) rather than reverting to stub mode at
// runtime.
func (s *State) ReloadConfigsIfChanged() error {
	...
	if errors.Is(err, os.ErrNotExist) {
		return nil // keep last-good; don't downgrade to stub at runtime
	}
	if err != nil {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	// Record the mtime either way so a bad version logs once, not on every
	// request; the operator's next edit bumps the mtime and retries.
...
// That's a real operational papercut this removes.
