...
// AttestationArm is the per-platform half of a policy: which verifier runs and
// which of the factors it reports must hold.
//
// It is split out because "block rooted/jailbroken devices" is ONE creator
// intent with two different mechanisms behind it. Android Key Attestation
// reports a verified-boot state and a device-lock bit; Apple App Attest reports
// neither (its keys are always hardware-backed and there is no boot-state
// concept), so a single flat set of require* gates cannot express the same
// intent on both platforms — demanding verified boot would reject every iPhone
// no matter how good its attestation.
type AttestationArm struct {
	...
	// Meaningful only for android-key-attestation; App Attest cannot satisfy it.
...
//
// Mode and the rate/TTL knobs are platform-independent — they encode what the
// creator asked for. The mechanism lives in Platforms, one [AttestationArm] per
// requester platform ("ANDROID", "IOS").
type AttestationPolicy struct {
	...
	// Platforms holds the per-platform arm, keyed by the uppercase platform
	// name a requester sends in AttestationBlob.Platform. A platform with no
	// arm cannot satisfy this policy because no verifier is configured for it.
	//
	// When empty, the deprecated flat fields below act as a single arm applied
	// to every platform, so entries written before per-platform arms existed
	// keep behaving exactly as they did.
	Platforms map[string]*AttestationArm `json:"platforms,omitempty"`

	// AttestationArm is the legacy single-arm form, inlined for compatibility
	// with configs.json files written before Platforms existed. Do not write
	// these on new entries; use Platforms.
	AttestationArm
}

// armFor returns the arm governing requests from platform, and whether one
// exists. Platform matching is case-insensitive because it arrives off the
// wire. A false second return means this policy has arms but none for that
// platform — the caller must treat the request as unattestable rather than
// falling back to another platform's rules.
func (p *AttestationPolicy) armFor(platform string) (*AttestationArm, bool) {
	if p == nil {
		return nil, false
	}
	if len(p.Platforms) == 0 {
		// Legacy flat policy: one arm, every platform.
		return &p.AttestationArm, true
	}
	if arm, ok := p.Platforms[strings.ToUpper(strings.TrimSpace(platform))]; ok && arm != nil {
		return arm, true
	}
	return nil, false
}
...
func loadConfigsFile(path string) (map[string]*ConfigEntry, error) {
	...
	for i := range list {
		...
		if len(cid) != envelopeConfigIDLen {
			return nil, fmt.Errorf(
				"entry %d (configId=%s): decodes to %d bytes; want %d (config IDs are generated by the dashboard)",
...
type RedemptionToken struct {
	...
	// ConfigIDs are minted together when this token is redeemed. A single
	// share link can therefore install a complete creator-selected set.
...
func (t RedemptionToken) configIDs() []string {
	return append([]string(nil), t.ConfigIDs...)
}

func validateRedemptionToken(entry RedemptionToken) error {
	if entry.Token == "" {
		return fmt.Errorf("missing token")
	}
	ids := entry.configIDs()
	if len(ids) == 0 {
		return fmt.Errorf("missing configIds")
	}
	seen := make(map[string]struct{}, len(ids))
	for _, id := range ids {
		if id == "" {
			return fmt.Errorf("empty configId")
		}
		if _, duplicate := seen[id]; duplicate {
			return fmt.Errorf("duplicate configId %s", shortBase64(id))
		}
		seen[id] = struct{}{}
	}
	return nil
...
// Each entry must carry a token and at least one configId, and tokens must be unique.
func loadRedemptionTokensFile(path string) (map[string]*RedemptionToken, error) {
	...
	for i := range list {
		entry := &list[i]
		if err := validateRedemptionToken(*entry); err != nil {
			return nil, fmt.Errorf("entry %d (token=%s): %w", i, shortBase64(entry.Token), err)
		}
