...
func renderUnitFile(o DeployOpts) string {
	o = o.withDefaults()
	return renderUnitFileWithExecStart(o, o.execStartLine())
}

// renderUnitFileWithExecStart applies the current hardening template around an
// already-resolved server command. Upgrade repair uses this to preserve every
// existing runtime flag exactly, including uncommon ports and explicit public
// URLs, while still moving the process to the isolated creator account.
func renderUnitFileWithExecStart(o DeployOpts, execStart string) string {
	o = o.withDefaults()
...
`, serviceUser, serviceUser, execStart, o.StateDir, capBounding, capAmbient)
}
...
// serviceController abstracts the dashboard's service-manager operations so
// the systemctl implementation can be substituted in tests.
...
// sudoNonInteractiveOK probes the exact private operation granted by the
// installer. A generic `sudo -n true` would fail under the deliberately narrow
// sudoers rule even though dashboard service access is correctly configured.
func sudoNonInteractiveOK() bool {
	if _, err := exec.LookPath("sudo"); err != nil {
		return false
	}
	name, args := sudoPermissionProbe(selfBinPath())
	return exec.Command(name, args...).Run() == nil
}

func sudoPermissionProbe(selfPath string) (string, []string) {
	return "sudo", []string{"-n", selfPath, "__service", "permission-check"}
}

// repairServiceUnit adopts the deployment settings from an existing unit and
// rewrites it through the current hardened template. This lets installer
// upgrades repair older root-run units without guessing the creator's domain,
// TLS mode, bind address, or state path.
func repairServiceUnit(ctrl serviceController, unitPath string, out io.Writer) error {
	data, err := os.ReadFile(unitPath)
	if err != nil {
		return fmt.Errorf("read existing service configuration: %w", err)
	}
	execStart := extractExecStartLine(string(data))
	if execStart == "" {
		return fmt.Errorf("cannot recover deployment settings from %s", unitPath)
	}
	opts := parseExecStart(execStart)
	if err := ctrl.Install(renderUnitFileWithExecStart(opts, execStart)); err != nil {
		return err
	}
	fmt.Fprintln(out, "repaired existing service configuration")
	return nil
}

// runServiceSubcommand handles the private privilege boundary used by the
// installer and dashboard. It is deliberately not a creator-facing command.
func runServiceSubcommand(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr,
			"internal service request is missing an operation")
		return 2
	}
	verb := args[0]
	if verb == "permission-check" {
		if os.Geteuid() == 0 {
			return 0
		}
		return 1
	...
	if verb == "update" {
		if os.Geteuid() != 0 {
			fmt.Fprintln(os.Stderr, "the dashboard updater requires installer-granted service access")
			return 1
		}
		cmd := exec.Command("sh", "-c", installOneLiner)
		cmd.Stdin, cmd.Stdout, cmd.Stderr = os.Stdin, os.Stdout, os.Stderr
		if err := cmd.Run(); err != nil {
			fmt.Fprintln(os.Stderr, "dashboard update failed:", err)
			return 1
		}
		return 0
	}
	if verb == "repair-existing" {
		if os.Geteuid() != 0 {
			fmt.Fprintln(os.Stderr, "service repair requires installer-granted access")
			return 1
		}
		ctrl := newSystemctlController()
		if err := repairServiceUnit(ctrl, ctrl.unitPath, os.Stdout); err != nil {
			fmt.Fprintln(os.Stderr, "service repair failed:", err)
			return 1
		}
		return 0
	}
	fs := flag.NewFlagSet("internal-service "+verb, flag.ContinueOnError)
	...
	if verb == "install" && opts.Domain == "" {
		fmt.Fprintln(os.Stderr, "setup requires a public domain")
		return 2
	}

	ctrl := newSystemctlController()
	if err := dispatchServiceVerb(ctrl, verb, opts, *logLines, os.Stdout); err != nil {
		fmt.Fprintf(os.Stderr, "service operation %s failed: %v\n", verb, err)
		return 1
	}
	return 0
}
