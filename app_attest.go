...
func (v *appleAppAttestVerifier) verifyWithAppID(blob AttestationBlob, appID string) (Verdict, error) {
	...
	verdict := Verdict{
		Verified:    trustedRoot,
		TrustedRoot: trustedRoot,
		// App Attest keys are always backed by dedicated hardware —
		// there's no software-only counterpart, so any successful
		// App Attest chain implies hardware backing. We surface it as "strongbox" for
