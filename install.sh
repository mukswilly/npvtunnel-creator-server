...
# reaches the privileged service verbs through `sudo creator-server service …`.
# This grants creator passwordless rights to ONLY that subcommand; every unit
...
creator ALL=(root) NOPASSWD: ${INSTALL_DIR}/${BIN} service *
EOF
$SUDO chmod 0440 "$sudoers"
# Reject our own drop-in if it doesn't parse, so a bad edit can't wedge sudo.
$SUDO visudo -cf "$sudoers" >/dev/null 2>&1 || { $SUDO rm -f "$sudoers"; echo "install: sudoers drop-in failed validation, removed"; }

# Write the systemd unit via the binary itself, so install.sh, the console, and
# the committed reference unit all come from ONE generator (no drift, no
# CHANGE_ME placeholder). Built-in TLS needs a domain up front; without one,
# defer unit creation to the console's setup wizard.
unit="/etc/systemd/system/creator-server.service"
if [ -n "$BUILTIN_TLS_DOMAIN" ]; then
  EMAIL_FLAG=""
  [ -n "$ACME_EMAIL" ] && EMAIL_FLAG="-acme-email ${ACME_EMAIL}"
  $SUDO "${INSTALL_DIR}/${BIN}" service install \
...
Almost there — finish setup in the console. It walks you through your domain
and TLS, writes the unit, starts the service, and shows the key to back up:
       sudo -u creator ${INSTALL_DIR}/${BIN} -state-dir ${STATE_DIR}
EOF
fi
