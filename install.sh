...
User=creator
Group=creator
ExecStart=${INSTALL_DIR}/${BIN} -addr 127.0.0.1:8443 -state-dir ${STATE_DIR} -public-issuer-url https://CHANGE_ME.example/v1/issue
Restart=on-failure
RestartSec=2
...
Next steps:
  ...
  2. Put a TLS-terminating reverse proxy (Caddy/nginx) in front of
     127.0.0.1:8443. See the deployment guide for a Caddy one-liner.
  3. Start it:
