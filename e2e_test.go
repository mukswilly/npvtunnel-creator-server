...
//     (no SHA-256-of-bytes anywhere — that's the bug 3.6g-1 fixes).
...
func TestEndToEndRedeemThenIssue(t *testing.T) {
	...
	// Recipient's hardware key (would be the Android Keystore / iOS
	// Secure Enclave key in production).
	...
	// The recipient's ECDH key for envelope unsealing. In the real
	// client this is a separate Keystore key; for the integration we
	// just need the pubkey to address the envelope to.
	...
	if issueResp.StatusCode != http.StatusOK {
		t.Fatalf("issue status %d: %s\n\n"+
			"This is the load-bearing 3.6g-4 assertion: redemption + "+
