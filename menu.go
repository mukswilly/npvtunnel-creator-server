...
func runMenuSubcommand(args []string) int {
	...
	if state != setupFirstRun && c.state.KeyWasCreated() {
		c.flash(fmt.Sprintf(
			"Your creator identity was created.\n\n"+
				"Creator pubkey:\n%s\n\n"+
				"Back up the state directory, including:\n%s",
...
func (c *console) showMain() {
	...
	list.AddItem("Import a config", "Paste a server registration exported by the app", '1', c.showAddConfig)
...
// showAddConfig accepts an app registration export, stores it, and reports the
// resulting configId and any device-attestation requirement.
func (c *console) showAddConfig() {
	...
	form.SetBorder(true).SetTitle(" Import a config ")

	help := tview.NewTextView().SetDynamicColors(true).SetText(
		"\n  Prepare the config and its restrictions in the app, copy its server\n" +
			"  registration, and paste it above. Raw config JSON is not accepted.")
	help.SetTextColor(tcell.ColorGray)

	body := tview.NewFlex().SetDirection(tview.FlexRow).
		AddItem(form, 0, 1, true).
		AddItem(help, 4, 0, false)
	c.switchTo("addconfig", "Import a config", footerKeys(""), body)
}
...
func (c *console) appendConfig(body json.RawMessage, rp registrationPolicy) (string, error) {
	...
	if rp.blockRooted {
		...
		// will be issued — Android Key Attestation, plus App Attest when the
		// registration carried an iOS app identity.
		entry.AttestationPolicy = strictDeviceAttestationPolicy(rp.iosAppID)
	}
...
func (c *console) showConfigs() {
	...
	if len(list) == 0 {
		c.flashThen("No configs yet.\n\nImport one from the main menu (option 1).", c.showMain)
		return
	}
...
// showMint renders the form for minting a redeemable share link for one or
// more configs. prefillID, when non-empty, preselects that config.
func (c *console) showMint(prefillID string) {
	...
	if len(configs) == 0 {
		c.flashThen("Import a config first (main menu, option 1).", c.showMain)
		return
	}
	selected := make([]bool, len(configs))
	...
	form.AddButton("Mint link", func() {
		...
		for i, checked := range selected {
			if checked {
				configIDs = append(configIDs, configs[i].ConfigID)
			}
		}
		link, err := c.mintShareLink(configIDs, strings.TrimSpace(redemptionURL), redemptions, expiresIn, label)
		if err != nil {
...
func (c *console) mintShareLink(configIDs []string, redemptionURL, redemptionsStr, expiresIn, label string) (string, error) {
	if len(configIDs) == 0 {
		return "", fmt.Errorf("select at least one config")
	}
	if redemptionURL == "" {
	...
	_, link, err := newShareLink(c.state, configIDs, redemptionURL, redemptions, expiresAt, strings.TrimSpace(label))
	if err != nil {
		return "", err
	}
	return link, nil
...
func (c *console) showTokens() {
	...
	table := tview.NewTable().SetFixed(1, 0).SetSelectable(true, false)
	for i, h := range []string{"TOKEN", "CONFIGS", "REMAIN", "EXPIRES", "LABEL", "STATUS"} {
		table.SetCell(0, i, headerCell(h))
	}
	for r, t := range list {
		...
		ids := t.configIDs()
		configSummary := "invalid"
		if len(ids) != 0 {
			configSummary = shortBase64(ids[0])
		}
		if len(ids) > 1 {
			configSummary += fmt.Sprintf(" +%d", len(ids)-1)
		}
		table.SetCell(r+1, 1, tview.NewTableCell(configSummary+"  "))
...
func (c *console) showServer() {
	...
	b.WriteString("\n  [white::b]Creator public key[-:-:-]\n")
	fmt.Fprintf(&b, "  pubkey     %s\n", c.state.CreatorPubKeyCompressedB64())
	fmt.Fprintf(&b, "  key file   %s  [gray](back up off-host)[-]\n", filepath.Join(c.stateDir, "creator-key.pem"))
	fmt.Fprintf(&b, "  registry   %d configs · %d live share links\n", len(configs), live)
	if !snap.Configured {
		b.WriteString("\n  [yellow]Not set up yet — run setup to configure your domain + TLS.[-]\n")
	}
	if !c.canManage {
		b.WriteString("\n  [gray]Service controls are unavailable. Re-run the installer as root\n" +
			"  to restore dashboard access to start, stop, and restart.[-]\n")
	}
...
// execServiceVerb crosses the installer's narrow privilege boundary for the
// dashboard's service controls.
...
func serviceVerbCommand(euid int, selfPath, verb string, extraArgs []string) (string, []string) {
	base := append([]string{"__service", verb}, extraArgs...)
	if euid == 0 {
	...
	go func() {
		...
		case err != nil:
			text = "Couldn't read logs:\n" + err.Error() +
				"\n\nRe-run the installer as root if dashboard log access is unavailable."
		case len(lines) == 0:
...
// terminal. The installer restarts an existing service after replacing the
// binary. The flow runs on a background goroutine and updates the UI through
// QueueUpdateDraw.
func (c *console) updateBinary() {
	c.confirm(
		"Update the creator-server binary?\n\n"+
			"This re-runs the official installer (verifies checksum + cosign\n"+
			"signature, replaces the binary, and restarts the service).\n"+
			"Your key + configs are untouched.",
		func() {
			go func() {
				var runErr error
				c.app.Suspend(func() {
					runErr = execServiceVerb("update", nil)
				})
				c.app.QueueUpdateDraw(func() {
					if runErr != nil {
					...
					c.flashThen("Updated. The service is now running the new binary.", c.showServer)
				})
			}()
		})
}
