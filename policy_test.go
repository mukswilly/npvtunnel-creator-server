...
// evalPolicy uses a fixed verifier so mode and TTL tests do not depend on
...
func evalPolicy(
	...
	d, _ := evaluateAttestationPolicy(p, att, func(string) (AttestationVerifier, error) {
		return v, nil
	}, ttl)
	return d
}
...
func TestPolicyOffIgnoresAttestation(t *testing.T) {
	for _, att := range []AttestationBlob{
		...
		d := evalPolicy(nil, att, nil, defaultConfigTtl)
		if d.reject {
	...
	d := evalPolicy(&AttestationPolicy{Mode: AttestationModeOff}, AttestationBlob{Platform: "NONE"}, nil, defaultConfigTtl)
...
func TestPolicyObserveLogsButDoesNotBlock(t *testing.T) {
	...
	for _, att := range []AttestationBlob{
		...
		d := evalPolicy(p, att, nil, defaultConfigTtl)
		if d.reject {
...
func TestPolicySoftShortensTtlWhenUnattested(t *testing.T) {
	...
	d := evalPolicy(p, AttestationBlob{Platform: "NONE"}, nil, defaultConfigTtl)
	if d.reject {
...
func TestPolicySoftFullTtlWhenAttested(t *testing.T) {
	p := &AttestationPolicy{Mode: AttestationModeSoft, SoftFailureTtlSec: 60}
	d := evalPolicy(p, AttestationBlob{Platform: "ANDROID", Token: "tok"}, nil, defaultConfigTtl)
	if d.ttl != defaultConfigTtl {
...
func TestIssueStrictModeRejectsRequestWithoutAttestation(t *testing.T) {
	...
	httpResp := postJSONForTest(t, ts.URL+"/v1/issue", body)
...
func TestPolicyHonorsBaseTtl(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			d := evalPolicy(tc.policy, tc.att, nil, base)
			if d.reject {
...
func TestPolicySoftFailureCappedAtBaseTtl(t *testing.T) {
	...
	d := evalPolicy(p, AttestationBlob{Platform: "NONE"}, nil, base)
	if d.ttl != base {
...
// A per-platform policy runs the arm matching the requester's platform: an
// iPhone is judged by the App Attest arm, not by Android's verified-boot gate.
func TestPolicyPerPlatformArmSelection(t *testing.T) {
	p := strictDeviceAttestationPolicy("TEAMID.com.example.app")

	android, ok := p.armFor("ANDROID")
	if !ok || android.Verifier != "android-key-attestation" || !android.RequireVerifiedBoot {
		t.Fatalf("android arm = %+v ok=%v", android, ok)
	}
	ios, ok := p.armFor("IOS")
	if !ok || ios.Verifier != "apple-app-attest" || ios.AppID != "TEAMID.com.example.app" {
		t.Fatalf("ios arm = %+v ok=%v", ios, ok)
	}
	// Verified boot is an Android concept; demanding it of App Attest would
	// reject every iPhone, so the iOS arm must not carry it.
	if ios.RequireVerifiedBoot {
		t.Error("ios arm must not require verified boot")
	}
	// Platform names arrive off the wire; matching is case-insensitive.
	if _, ok := p.armFor("ios"); !ok {
		t.Error("armFor must match platform names case-insensitively")
	}
}

// Without an app identity there can be no iOS arm, and strict mode then turns
// iPhones away with a reason naming the platform rather than a verifier error.
func TestPolicyStrictRejectsPlatformWithNoArm(t *testing.T) {
	p := strictDeviceAttestationPolicy("")
	if _, ok := p.armFor("IOS"); ok {
		t.Fatal("no appId must mean no iOS arm")
	}

	d := evalPolicy(p, AttestationBlob{Platform: "IOS", Token: "tok"}, nil, defaultConfigTtl)
	if !d.reject {
		t.Fatalf("strict must reject a platform it has no rule for: %+v", d)
	}
	if !strings.Contains(d.rejectReason, "no attestation rule for platform") {
		t.Errorf("rejectReason = %q, want it to name the missing platform rule", d.rejectReason)
	}
	// The Android arm still works.
	if d := evalPolicy(p, AttestationBlob{Platform: "ANDROID", Token: "tok"}, nil, defaultConfigTtl); d.reject {
		t.Errorf("android must still be judged on its own arm: %+v", d)
	}
}

// An unarmed platform is not attested, so soft mode shortens its TTL rather
// than granting the full one, and observe mode still never rejects.
func TestPolicyUnarmedPlatformInSoftAndObserve(t *testing.T) {
	arms := strictDeviceAttestationPolicy("").Platforms

	soft := &AttestationPolicy{Mode: AttestationModeSoft, SoftFailureTtlSec: 60, Platforms: arms}
	if d := evalPolicy(soft, AttestationBlob{Platform: "IOS", Token: "tok"}, nil, defaultConfigTtl); d.ttl != 60*time.Second {
		t.Errorf("soft + unarmed platform: ttl = %v, want the soft-failure ttl", d.ttl)
	}

	observe := &AttestationPolicy{Mode: AttestationModeObserve, Platforms: arms}
	if d := evalPolicy(observe, AttestationBlob{Platform: "IOS", Token: "tok"}, nil, defaultConfigTtl); d.reject {
		t.Error("observe must never reject, even for an unarmed platform")
	}
}

// Entries written before per-platform arms existed keep working: the flat
// fields act as one arm covering every platform.
func TestPolicyLegacyFlatArmStillApplies(t *testing.T) {
	legacy := &AttestationPolicy{
		Mode:           AttestationModeStrict,
		AttestationArm: AttestationArm{Verifier: "android-key-attestation", RequireTrustedRoot: true},
	}
	for _, platform := range []string{"ANDROID", "IOS"} {
		arm, ok := legacy.armFor(platform)
		if !ok || arm.Verifier != "android-key-attestation" || !arm.RequireTrustedRoot {
			t.Errorf("legacy arm for %s = %+v ok=%v", platform, arm, ok)
		}
	}
	// And it round-trips through the on-disk shape unchanged.
	blob, err := json.Marshal(legacy)
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
	if !strings.Contains(string(blob), `"verifier":"android-key-attestation"`) {
		t.Errorf("legacy policy must keep its flat shape on disk: %s", blob)
	}
	var back AttestationPolicy
	if err := json.Unmarshal(blob, &back); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if arm, ok := back.armFor("ANDROID"); !ok || arm.Verifier != "android-key-attestation" {
		t.Errorf("legacy policy did not round-trip: %+v", back)
	}
}

// A policy naming a verifier the registry can't resolve is a server
// misconfiguration and surfaces as an error, not as a silent rejection.
func TestPolicyUnknownVerifierIsAnError(t *testing.T) {
	p := &AttestationPolicy{
		Mode:           AttestationModeStrict,
		AttestationArm: AttestationArm{Verifier: "nonesuch"},
	}
	reg := newVerifierRegistry()
	if _, err := evaluateAttestationPolicy(
		p, AttestationBlob{Platform: "ANDROID", Token: "tok"}, reg.Lookup, defaultConfigTtl,
	); err == nil {
		t.Fatal("unknown verifier must return an error")
	}
}
